...
export default function checkInnerEffects(phase: EffectPhase = "endOfStep", stepName: string = "default"): string[] {
...
function didCheckChange(check: InnerEffectCheck<any, any>, diffInfo: typeof meta.diffInfo) {
  const editedCheck = {
  ...
  if (check.addedOrRemoved) {
    for (let typesIndex = 0; typesIndex < editedCheck.types.length; typesIndex++) {
      ...
      if (
        (editedCheck.names[0] === "all__" && diffInfo.itemsAdded[itemType].length > 0) ||
      ...
      for (let namesIndex = 0; namesIndex < editedCheck.names.length; namesIndex++) {
        ...
        if (
          (diffInfo.itemsAddedBool[itemType] && diffInfo.itemsAddedBool[itemType][loopedItemName]) ||
