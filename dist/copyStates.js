...
export function copyChangedStates(nowState, prevState /* saveToObject */) {
    ...
    for (let typeIndex = 0; typeIndex < itemTypesWithAdded.length; typeIndex++) {
        ...
        for (let idIndex = 0; idIndex < itemIds.length; ++idIndex) {
            const itemId = itemIds[idIndex];
            if (!prevState[itemType][itemId])
                prevState[itemType][itemId] = meta.newStateByItemType[itemType](itemId);
        }
    }
    // Check changes here
